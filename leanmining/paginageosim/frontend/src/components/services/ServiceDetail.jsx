...
import { getServiceTranslationKey } from '../../i18n/serviceI18n';

const ServiceDetail = ({ service, index, isReversed }) => {
  const { t, i18n } = useTranslation();
  const isEnglish = i18n.language?.startsWith('en');
  const translationKey = getServiceTranslationKey(service.id);
  const i18nBase = translationKey ? `servicesPage.services.${translationKey}` : null;
  const localizedFeatures = service.features.map((feature, featureIndex) =>
    isEnglish && i18nBase
      ? t(`${i18nBase}.feature${featureIndex + 1}`, { defaultValue: feature })
      : feature
  );

  const bgClass =
    index % 2 === 0 ? 'bg-white' : 'bg-gradient-to-br from-slate-50 via-white to-slate-50';
  ...
  return (
    <section id={service.id} className={`relative py-24 md:py-32 ${bgClass} overflow-hidden`}>
      ...
      <Container className="relative">
        <div
          className={`grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center ${isReversed ? 'lg:flex-row-reverse' : ''}`}
        >
          <motion.div
          ...
          >
            ...
            <div
              className={`absolute -z-10 w-full h-full bg-gradient-to-br from-primary-100 to-primary-50 rounded-3xl top-6 ${isReversed ? '-left-6' : '-right-6'}`}
            />
          </motion.div>
          ...
          >
            <span className="inline-flex items-center gap-2 px-4 py-2 bg-primary-50 text-primary-600 text-sm font-semibold rounded-full mb-6 border border-primary-100">
              ...
              {t('common.service')} {String(index + 1).padStart(2, '0')}
            </span>
